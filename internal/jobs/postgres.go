...
func (r *PostgresRepository) mapError(op string, err error) error {
	...
	if errors.As(err, &pqErr) {
		...
		// pqErr.Constraint indicates which unique constraint fired,
		// useful if a table has multiple. Log it here before discarding
		// the raw pq.Error, since it won't propagate further.
